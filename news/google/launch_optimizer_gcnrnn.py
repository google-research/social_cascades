...
"""Launcher for main_comment_gcn.py on PyTorch with GPUs using JobTrialGenerator.
...
FLAGS = flags.FLAGS
flags.DEFINE_string('project_name', 'gcnrnn', 'name for the project')
...
flags.DEFINE_float('rnn_dropout', 0.3, 'dropout rate for RNN/LSTM')
...
def main(_):
  ...
  if FLAGS.image_uri:
    ...
    executable = xm.CloudPython(
        ...
        module_name='gnns_for_news.main_comment_gcn',
  ...
  parameters = [
      ...
      hyper.get_vizier_parameter_config(
          'batch_size', hyper.discrete([8 * k for k in range(1, 6)])),
      hyper.get_vizier_parameter_config(
          'gcn_hid_dim', hyper.discrete([16 * k for k in range(3, 10)])),
      hyper.get_vizier_parameter_config(
          'gcn_out_dim', hyper.discrete([16 * k for k in range(3, 10)])),
      hyper.get_vizier_parameter_config(
          'rnn_hid_dim', hyper.discrete([16 * k for k in range(3, 10)])),
      hyper.get_vizier_parameter_config(
          'rnn_num_layers', hyper.discrete([1, 2])),
      hyper.get_vizier_parameter_config(
          'lr_gcn', hyper.interval(0.00001, 0.2), scaling='UNIT_LOG_SCALE'),
      hyper.get_vizier_parameter_config(
          'lr_rnn', hyper.interval(0.00001, 0.2), scaling='UNIT_LOG_SCALE'),
      hyper.get_vizier_parameter_config(
          'rnn_dropout', hyper.discrete([0.0, 0.15, 0.3, 0.5, 0.7])),
      hyper.get_vizier_parameter_config(
          'gcn_dropout', hyper.discrete([0.0, 0.15, 0.3, 0.5, 0.7])),
      hyper.get_vizier_parameter_config(
